...
class PlayState extends FlxState
{
	...
	override public function create():Void
	{
		#if newgrounds
		if (!NGio.isLoggedIn)
			var ng = new NGio(APIStuff.API, APIStuff.EncKey);
		// NGio.init();
		// NGio.login();
		#end
		...
		_map.foregroundTiles.forEach(function(t:FlxTilemap)
		{
			t.visible = false;
		});
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (_player.x < 0 - _player.width)
		{
			FlxG.camera.fade(FlxColor.WHITE, 0.5, false, function()
			{
				...
				NGio.postScore(Std.int(_timer * 1000 * 1000), "Fastest Completion");
